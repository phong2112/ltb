...
/** Normalizes OCR output while preserving meaningful CV line breaks. */
...
/** Cleans, redacts, and limits CV text before sending it to an AI provider. */
...
/** Builds a criterion-focused, redacted CV input for AI matching. */
...
/** Scores how readable and usable extracted CV text on a 0–100 scale. */
...
/** Applies cleanup, PII redaction, and optional criterion curation to CV text. */
function prepareCvText(
  ...
  /** Redacts valid phone-number matches while tracking their count. */
...
/** Escapes plain text for safe use in a regular expression. */
function escapeRegExp(value: string) {
  return value.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&");
}

/** Collapses consecutive duplicate non-empty lines. */
...
/** Normalizes a line so duplicate lines can be compared consistently. */
function normalizeComparableLine(value: string) {
  return value.toLocaleLowerCase("vi").replace(/\s+/g, " ").trim();
}

/** Removes page labels, symbol noise, and repeated non-essential CV lines. */
...
/** Omits standalone PII placeholders from AI matching input. */
...
/** Creates a compact CV pack organized around the matching criteria. */
...
/** Splits the CV into recognized semantic sections. */
...
/** Selects and truncates the most useful CV sections for matching. */
...
/** Finds the strongest evidence snippets for every match criterion. */
...
/** Builds contextual searchable chunks from non-heading CV lines. */
...
/** Associates each source line index with its detected CV section. */
...
/** Scores a CV chunk by keyword relevance and section priority. */
...
/** Extracts distinct, meaningful keywords from a criterion. */
...
/** Keeps only the first occurrence of each normalized snippet. */
...
/** Returns the character budget for a CV section in the curated pack. */
...
/** Removes blank lines from the beginning and end of a section. */
...
/** Detects lines that are overwhelmingly punctuation or symbols. */
...
/** Preserves repeated lines that likely contain meaningful CV content. */
...
/** Truncates text while retaining both its beginning and ending context. */
