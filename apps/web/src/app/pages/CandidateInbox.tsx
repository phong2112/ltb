...
export default function CandidateInbox() {
  const { candidateProfiles, jobs, reloadAdminData } = useData();
  ...
  const filtered = candidateProfiles.filter(candidate => {
    ...
    const matchStatus =
      statusFilter === "all" ||
      candidate.applications.some(application => application.status === statusFilter);
    const matchJob =
      jobFilter === "all" ||
      candidate.applications.some(application => application.jobId === jobFilter);
    ...
    <AdminLayout>
      <div className="mb-6">
        ...
        <p className="text-muted-foreground text-sm mt-0.5">
          {candidateProfiles.length} {t("admin.totalCandidatesSummary")} ·{" "}
        ...
        {filtered.length === 0 ? (
          ...
          <div className="divide-y divide-border">
            {paginatedCandidates.map(candidate => {
              ...
              const latestApplication = filteredApplications[0] ?? candidate.applications[0];

              return (
                <Link
                  key={candidate.id}
                  to={`/admin/candidates/${candidate.id}${latestApplication ? `?application=${latestApplication.applicationId}` : ""}`}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {candidate.applications.some(a => a.status === "new") && (
                        <span className="w-1.5 h-1.5 rounded-full bg-blue-500 flex-shrink-0" />
                      )}
                      <span className="rounded-full bg-secondary px-2 py-0.5 text-[10px] font-bold text-muted-foreground">
                        {candidate.applications.length} {t("admin.applications")}
                      </span>
                    </div>
                    <p className="text-xs text-muted-foreground truncate">
                      {latestApplication?.jobTitle ?? "—"} · {candidate.email || "—"}
                    </p>
                  </div>
                  {latestApplication && (
                    <div className="hidden sm:flex items-center gap-3 flex-shrink-0">
                      <span className={`rounded-full border px-2 py-1 text-[10px] font-bold ${CANDIDATE_STATUS_CONFIG[latestApplication.status].badgeClass}`}>
                        {translateCandidateStatus(latestApplication.status, language)}
                      </span>
