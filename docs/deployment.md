...
Candidate application confirmation email is optional and uses the **Gmail API over HTTPS** (OAuth2). This sends from your `@gmail` account without needing a verified domain, and works on hosts that block outbound SMTP ports (e.g. Render's free tier). See [docs/gmail-api-email-setup.md](gmail-api-email-setup.md) for how to obtain the OAuth2 refresh token, then set:
...
Confirmation emails use `WEB_ORIGIN` to build public job links and the logo image URL, so set `WEB_ORIGIN` to the deployed web domain in staging/production. If `EMAIL_FROM`, `GMAIL_CLIENT_ID`, `GMAIL_CLIENT_SECRET`, or `GMAIL_REFRESH_TOKEN` is blank, the API still accepts applications but skips confirmation email.
